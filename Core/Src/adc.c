...
//			if(chx->enabled)
//				ADC1->CFGR = 0x00003001; // Continuous mode, overrun enabled, 12bit, right align, DMA one-shot, enabled
			switch(dso1.timeDiv){
				case timeDiv_1ms:{ // 1 ms
					ADC1->SMPR1 = 0x00000038; // => sampling time 614 cycles (117.264 KS/s)
					break;
				}
				case timeDiv_500us:{// 500 us
					ADC1->SMPR1 = 0x00000030; // => sampling time 194 cycles (371.31 KS/s)
					break;
				}
				case timeDiv_250us:{//250 us
					ADC1->SMPR1 = 0x00000028; // => sampling time 74 cycles (973 KS/s)
					break;
				}
				case timeDiv_100us:{// 100 us
					ADC1->SMPR1 = 0x00000018; // => sampling time 20 cycles (3.6 MS/s)
					break;
				}
				case timeDiv_50us:{//50 us
					ADC1->SMPR1 = 0x00000000; // => sampling time 14 cycles (5.14 MS/s)
					break;
				}
				case timeDiv_25us:{//25 us
					ADC1->SMPR1 = 0x00000000; // => sampling time 14 cycles (5.14 MS/s)
					break;
				}
				case timeDiv_10us:{//10 us
					ADC1->SMPR1 = 0x00000000; // => sampling time 14 cycles (5.14 MS/s)
					break;
				}
				case timeDiv_5us:{//5 us
					ADC1->SMPR1 = 0x00000000; // => sampling time 14 cycles (5.14 MS/s)
					break;
				}
				case timeDiv_2_5us:{//2.5 us
					ADC1->SMPR1 = 0x00000000; // => sampling time 14 cycles (5.14 MS/s)
					break;
				}
				case timeDiv_1us:{//1 us
					ADC1->SMPR1 = 0x00000000; // => sampling time 14 cycles (5.14 MS/s)
					break;
				}
				default:{
...
// ch1/ch2 in regular simultaneous or ch3/ch4 in regular simultaneous
void setupADCParallel(uint32_t num){ // num = 0 -> adc1_2, num = 1 -> adc3_4
	// dual mode used for 10 MS/s only (nanosecond timeDiv)
	if(num){// ADC3_4 in dual
		disableADC(3);
		disableADC(4);
...
//		ADC2->CR |= ADC_CR_ADDIS; // disable ADC2
		overrun = 1;
...
//		ADC4->CR |= ADC_CR_ADDIS; // disable ADC4
