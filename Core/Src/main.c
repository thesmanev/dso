...
uint32_t channel_state = 0;				/**< Indicates which of the four channels is enabled */
...
{
	uint8_t str[7] = { 'e', 'r', 'r', 'o', 'r', ' ', ' ' };
	str[6] = (uint8_t)(errno + 48);
...
void initDSO(void)
{
	...
	dso.ch2.coupling = 0; //dc
	dso.ch2.enabled = 0; // off
	dso.ch2.selected = 0;
	dso.ch2.id = 2;
	...
	dso.ch3.coupling = 0; //dc
	dso.ch3.enabled = 0; // off
	dso.ch3.selected = 0;
	dso.ch3.id = 3;
	...
	dso.ch4.coupling = 0; //dc
	dso.ch4.enabled = 0; // off
	dso.ch4.selected = 0;
	dso.ch4.id = 4;
